...
func PieChartForEngineers(title string, records [][]string) *charts.Pie {
	pie := charts.NewPie()
	pie.AddSeries("pie", pieDataForEngineers(records))

	titleOpts := opts.Title{
		Title:    "Hours per Engineer",
		Subtitle: title,
	}
...
func pieDataForEngineers(records [][]string) []opts.PieData {
	entries := massagePieDataForEngineers(records)
...
func massagePieDataForEngineers(records [][]string) map[string]float64 {
	...
	for index, record := range records {
		...
		noteRaw := fmt.Sprintf("%s %s", record[10], record[11])
		hoursRaw := record[6]
